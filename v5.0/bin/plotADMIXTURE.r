...
tiff(file=paste0(opt$outPrefix,".tiff"),width = 3000, height = 1200,res=200)
 ...
 axis(3,at=bp,labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.5)
 ...
 axis(1,at=c(which(spaces==0.5),bp[length(bp)])-diff(c(1,which(spaces==0.5),bp[length(bp)]))/2,
     #labels=unlist(strsplit(opt$populations,",")),tick=F)
     labels=F,tick=F
     #labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.6
      )
dev.off()

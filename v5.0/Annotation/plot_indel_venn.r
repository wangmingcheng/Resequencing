...
spec <- matrix(c(
				...
				'name', 'n', 1, "character", "output file prefix, INDEL."
		), byrow = TRUE, ncol = 5)
...
if ( is.null(opt$name)){opt$name="INDEL"}
...
#
#
#p1<-ggplot(depthData,aes(x=x,y=y*100,color=sample)) + geom_line()+xlab("Depth") + ylab("SNP Fraction(%)")+labs(title="SNP depth distribution")
#
...
#}
#p<-ggplot(distanceData,aes(x=x,y=y*100,color=sample)) + geom_line()+xlab("Neighboring SNP distance(bp)") + ylab("SNP Fraction(%)")+labs(title="Neighboring SNP distance distribution")
#p <- p +theme_bw()+ theme(  
#				panel.grid=element_blank(), 
#				axis.text.x=element_text(colour="black"),axis.text.y=element_text(colour="black"),panel.border=element_rect(colour = "black"))+
#		#scale_colour_manual(values = c(mycol[1],mycol[2],mycol[3],mycol[4],"grey"))+

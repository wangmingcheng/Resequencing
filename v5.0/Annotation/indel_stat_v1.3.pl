...
my %opts;
GetOptions(\%opts,"i=s","o=s","m=s","x=s","maxporc=s","s=s","h" );
...
#	close(OUT);
	...
	print OUT "#Length\tregion\tNumber\tSample\n" ;
	for my $len (sort {$a<=>$b} keys %{$ahstat->{'all'}->{'genomic'}}){
		...
		print OUT "$len\tGenome\t$genome_num\t$opts{s}\n" ;
		print OUT "$len\tCDS\t$cds_num\t$opts{s}\n" ;
	}
	close(OUT);

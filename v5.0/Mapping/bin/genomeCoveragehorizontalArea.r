...
--level         -e      character       reference genome assemble level(chr or scaffold) [forced]
\n")
	q(status=1);
}
...
if ( is.null(opt$level) )       { print_usage(spec) }
...
# reading id
id <- read.table(opt$idfile, stringsAsFactors = F, header = F)[1]
id
...
id <- as.data.frame(id[multi.mixedorder(id$V1),])
names(id) <- "V1"
if(nrow(id) > 30 ){
	if(opt$level == "chr"){
		print("Reference assemble level: chr")
	}else{
		id <- as.data.frame(id[1:30,])
		print("Reference assemble level: scaffold")
	}
}
# check dim
id.dim <- dim(id)
...
df <- data.frame(x=data[,opt$x.col], group=factor(data[,opt$group.col], levels=id[,1]), y=data[,opt$y.col])

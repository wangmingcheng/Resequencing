...
&help() if ( defined($opts{h}) || !defined($opts{i}) ||!defined($opts{o}) || !defined($opts{k}) || !defined($opts{d}) );
...
$opts{map} ||= "/share/nas2/database/KEGG/20150511/20150511/";
$opts{func} ||= "both";
...
unless ($opts{func} eq "kegg") 
{
	...
	system "perl $Bin/gene_ontology_graph_v1.pl -i $opts{i}/$go_list -i $opts{k}.GO.list.txt -mark All_gene -mark Var_gene -o $go_out -k $opts{k}" ; 
...
#		print OUT "\#$_";
		...
		print OUT "\#Go_Term",'GO_ID','Variant_gene','gene','Variant_gene_all','gene_all';
...
unless ($opts{func} eq "go")
{
	...
	print OUT "\#Pathway",'ko_ID','Variant_gene','gene','Variant_gene_all','gene_all';
#---------------------------------- by Simon Young 2015-03-04 --------------------------------------
        my $header =<<"_HEADER_";
...
#        print OUT $header;
	...
	for my $koxx (sort keys %png_new) {
		if (-e "$opts{map}/$koxx.png") {
			...
			my $im = newFromPng GD::Image(\*PNG,1) ||die;		
						...
						if($type=~/rect/){
							...
							for(my $i=1;$i<=$#temp-1;$i++){
								...
								if($p2==$p4){
									...
									$html{$koxx}{"$p1,$p2,$p3,$p4"}{png_coord}="$p1,$p2,$p3,$p4,$p3,$y1,$p1,$y1,$p1,$p2";  
									$html{$koxx}{"$p1,$p2,$p3,$p4"}{html_coord}="$p1,$y1,$p1,$y2,$p3,$y4,$p3,$y3,$p1,$y1"; 
								}
								else{
...
____________HTML
			foreach my $coords ( keys %{$html{$koxx}} ) {
				...
				if (!($updown eq 'Mix')) {
					print HTML <<____________HTML;
<area shape='poly' coords='$html{$koxx}{$coords}{html_coord}' href='http://www.kegg.jp/dbget-bin/www_bget?$html{$koxx}{$coords}{KO}' target="_blank" onmouseover='javascript: showInfo("<ul><li style=\\"color: #$color;\\">variant$info</li></ul>");' />
____________HTML
				}else{

...
sub variants_calling_by_gatk() 
{
	...
	my $shfile = "$dir/$outprefix.0.gatk_var_call.sh";
	my $sample_num = @{$abamfiles};	
	my ($gatk_thread, $qsub_proc)=(5,$maxcpu);
	my @list_files = &get_chr_list_file($dir,50);
	my $cmd = "";

#calling variants use HaplotypeCaller

		open (SH, ">$shfile") || die "Can't open $shfile, $!\n" ;
		my $combinesh = "$dir/../cohort/$outprefix.1.CombineGVCFs.cohort.sh" ;
		open (SH1, ">$combinesh") || die "Can't open $combinesh, $!\n" ;
		my $genotypesh = "$dir/../cohort/$outprefix.2.GenotypeGVCFs.cohort.sh" ;
		open (SH2, ">$genotypesh") || die "Can't open $genotypesh, $!\n" ;
		
		my $rmsh = "$dir/../cohort/bye_gvcf.sh" ;
		open (SH3, ">$rmsh") || die "Can't open $rmsh, $!\n" ;
		my$comNum=10;
		my @vcf_files = () ;
		for (my $i=1; $i<=@list_files; $i++){
			...
			my $basename = "$outprefix.$i.$j.g.vcf";
			my $outfile = "$dir/$basename" ;
				print SH "$java  -XX:ParallelGCThreads=5  -Djava.io.tmpdir=$tmp_dir -Xmx10G -jar $gatk_dir/GenomeAnalysisTK.jar -T HaplotypeCaller -R $ref --indelSizeToEliminateInRefModel 50 --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 --sample_ploidy $ploidy  " ;
				print SH "  -nct $thread_num -o $outfile  -L $list_file" ;
				push @GVCF, " --variant $outfile " ;
				print SH3 "rm $outfile\n";
				print SH3 "rm $outfile.idx\n";
				...
				if($j!=0 and $j % $comNum ==0){
					print SH1 "$java  -XX:ParallelGCThreads=5  -Djava.io.tmpdir=$tmp_dir -Xmx30G -jar $gatk_dir/GenomeAnalysisTK.jar -T CombineGVCFs -R $ref --disable_auto_index_creation_and_locking_when_reading_rods -o $dir/../cohort/cohort.$i.$j.$k.g.vcf @GVCF\n";
					push @comGVCF," --variant $dir/../cohort/cohort.$i.$j.$k.g.vcf ";
					print SH3 "rm $dir/cohort.$i.$j.$k.g.vcf\n";
					print SH3 "rm $dir/cohort.$i.$j.$k.g.vcf.idx\n";
					$k++;
					@GVCF=();
			...
			if(@GVCF >1){
				print SH1 "$java  -XX:ParallelGCThreads=5  -Djava.io.tmpdir=$tmp_dir -Xmx30G -jar $gatk_dir/GenomeAnalysisTK.jar -T CombineGVCFs -R $ref --disable_auto_index_creation_and_locking_when_reading_rods -o $dir/../cohort/cohort.$i.x.0.g.vcf @GVCF\n";
				push @comGVCF," --variant $dir/../cohort/cohort.$i.x.0.g.vcf ";

				# 		print SH1 "$java -XX:ParallelGCThreads=5  -Djava.io.tmpdir=$tmp_dir -Xmx30G -jar $gatk_dir/GenomeAnalysisTK.jar -T CombineGVCFs -R $ref --disable_auto_index_creation_and_locking_when_reading_rods -o $dir/cohort.$i.g.vcf @GVCF \n";
# 		push @comGVCF," --variant $dir/cohort.$i.g.vcf ";
# 		print SH2 "$java  -XX:ParallelGCThreads=5  -Djava.io.tmpdir=$tmp_dir -Xmx50G -jar $gatk_dir/GenomeAnalysisTK.jar -T GenotypeGVCFs -nt 1 -R $ref --disable_auto_index_creation_and_locking_when_reading_rods -o $dir/combine.$i.snp.indel.vcf @comGVCF \n";

				print SH3 "rm $dir/cohort.$i.x.0.g.vcf\n";
				print SH3 "rm $dir/cohort.$i.x.0.g.vcf.idx\n";
			}elsif(@GVCF==1){
				$GVCF[0]=~s/ --variant//g;
				chomp $GVCF[0];
				$GVCF[0] =~ s/\s+$//;
				print "\"$GVCF[0]\"\n";
				print SH1 "cp  $GVCF[0]  $dir/cohort.$i.x.0.g.vcf\n";
				print SH1 "cp  $GVCF[0].idx  $dir/cohort.$i.x.0.g.vcf.idx\n";
				push @comGVCF," --variant $dir/cohort.$i.x.0.g.vcf ";
				print SH3 "rm $dir/cohort.$i.x.0.g.vcf\n";
				print SH3 "rm $dir/cohort.$i.x.0.g.vcf.idx\n";
			}
			
			print SH2 "$java  -XX:ParallelGCThreads=5  -Djava.io.tmpdir=$tmp_dir -Xmx50G -jar $gatk_dir/GenomeAnalysisTK.jar -T GenotypeGVCFs -nt 4 -R $ref --disable_auto_index_creation_and_locking_when_reading_rods -o $dir/combine.$i.snp.indel.vcf @comGVCF \n";
			push @vcf_files, "$dir/combine.$i.snp.indel.vcf";
			print SH3 "rm $dir/combine.$i.snp.indel.vcf\n";
			print SH3 "rm $dir/combine.$i.snp.indel.vcf.idx\n";
		}
		...
		#&qsub($shfile, $opts{queue}, $max); #modified by wangyt in 2019.4.10
		# &qsub($combinesh, $opts{queue}, $qsub_proc);
		# &qsub($genotypesh, $opts{queue}, $qsub_proc);
	&qsub($combinesh, $queue, $maxcpu);
    &qsub($genotypesh, $queue, $maxcpu);

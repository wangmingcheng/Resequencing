...
sub qsub()
{
	...
	my $cmd = "cd $dir && /share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh --maxproc $ass_maxproc --queue $queue --resource vf=50G --reqsub $shfile --independent" ;
	&run_or_die($cmd);
	return ;
}

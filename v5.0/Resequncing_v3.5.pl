...
`mkdir -p $od/genome_stat` unless (-d "$od/genome_stat/");
...
foreach my $samp (sort keys %seq_hash){
    ...
    print FASTP "cd $od/00.data_clean/$samp && $config{fastp} -i $seq_hash{$samp}{1} -I $seq_hash{$samp}{2} -o $samp\_clean.1.fq.gz -O $samp\_clean.2.fq.gz --thread 7 --json $samp.json --html $samp.html\n";                
    ...
    print MD "$config{gatk} MarkDuplicates -I $od/02.bwa_samtools/$samp.sorted.bam -M $od/03.gatk_MarkDuplicates/$samp\_duplication.metrics -O $od/03.gatk_MarkDuplicates/$samp.MarkDups\_sorted.bam && $config{samtools} index -@ 10 $od/03.gatk_MarkDuplicates/$samp.MarkDups\_sorted.bam && $config{samtools} coverage $od/03.gatk_MarkDuplicates/$samp.MarkDups\_sorted.bam -o $od/03.gatk_MarkDuplicates/$samp.MarkDups\_sorted.bam.coverage && $config{samtools} flagstat $od/03.gatk_MarkDuplicates/$samp.MarkDups\_sorted.bam > $od/03.gatk_MarkDuplicates/$samp.MarkDups\_sorted.bam.stat\n";
...
#genome coverage,depth,mapping rate stat
open (GS,">$od/genome_stat/genome_stat.sh");
print GS "perl $Bin/bin/bam_stat.pl $od/03.gatk_MarkDuplicates\n";
